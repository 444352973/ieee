...
def predict_check(pred_variable, gold_variable, mask_variable):
    ...
    """
    pred = pred_variable.cpu().numpy()
    gold = gold_variable.cpu().numpy()
    mask = mask_variable.cpu().numpy()
    overlaped = (pred == gold)
...
def recover_label(pred_variable, gold_variable, mask_variable, label_alphabet, word_recover):
    ...
    mask = mask_variable.cpu().numpy()
    pred_tag = pred_variable.cpu().numpy()
    gold_tag = gold_variable.cpu().numpy()
...
def batchify_with_label(input_batch_list, gpu, volatile_flag=False):
    ...
    word_seq_lengths = torch.LongTensor([len(words[0])])
    ...
    mask=mask/int(list(mask.shape)[1])
    if gpu:
...
def train(data, save_model_dir, seg=True):
    ...
    for idx in range(data.HP_iteration):
        ...
        fw.write(str(idx)+'    ')
        fw.write(str(dt.strftime( '%Y-%m-%d %H:%M:%S %f' )))
        fw.write('\n')
        fw.close()
...
if __name__ == '__main__':
    ...
    char_emb = "data/ctb.50d.vec"
    bichar_emb = None
    gaz_file = "data/gigaword_chn.all.a2b.uni.ite50.vec"
    # gaz_file = None
    # char_emb = None
    #bichar_emb = None
